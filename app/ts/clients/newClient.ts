...
export async function onClient(name: string, email: string, phone: string, address: string, navigation: any) {
    ...
    try {
        ...
        await fetchAndUpdateClients(access_token);
...
export async function getClients() {
    try {
        ...
        if (cachedClients) {
            ...
            fetchAndUpdateClients(access_token);

            return clients;
        } else {
            // Fetch clients if no cached data is available
            return await fetchAndUpdateClients(access_token);
        }
    } catch (error) {
        console.error(error);
        const err = error as ClientError;
...
export async function fetchAndUpdateClients(access_token: string) {
    const apiurl = Constants.expoConfig?.extra?.API_URL;
    if (!apiurl) {
        throw new Error('API_URL not found');
    }
